...
function barsHtml(items,total){return items.map(([name,n])=>`<div class="bar-row"><div class="bar-label"><span>${safe(name)}${total?`<small>${pct(n,total)}</small>`:''}</span><strong>${fmt(n)}</strong></div><div class="bar-track"><div class="bar-fill" style="width:${total?Math.min(100,n/total*100):0}%"></div></div></div>`).join('')}
...
function renderResult(r){
  ...
  const seasonCases=r.caseAnalysis.cases.length,over500=r.sccBuckets.over500,monthsOver=r.caseAnalysis.monthly.filter(x=>x.overTrigger).length,early30=r.caseAnalysis.timing.d0_7+r.caseAnalysis.timing.d8_30;
