...
function renderResult(r){
  ...
  if(state.dct){
    if(pp.length){
      ...
      const comboOutcome=comboDen?`${fmt(r.prevComboMastitis)} / ${fmt(comboDen)} · ${pct(r.prevComboMastitis,comboDen)}`:'N/A';
      const sealOutcome=sealDen?`${fmt(r.prevSealantOnlyMastitis)} / ${fmt(sealDen)} · ${pct(r.prevSealantOnlyMastitis,sealDen)}`:'N/A';
