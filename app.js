...
function buildCaseAnalysis(records,herdById,seasonYear,peak,firstCalvers){
  ...
  for(const r of mastitisRows){const key=`${r.id}|${dateKey(r.date)||'nodate-'+r.index}`;if(!groups.has(key))groups.set(key,{id:r.id,date:r.date,rows:[],quarters:new Set(),antibiotics:new Set(),nsaid:false,maxDoses:0,forceClinicalCase:false,historic:false});const g=groups.get(key);g.rows.push(r);if(r.forceClinicalCase)g.forceClinicalCase=true;if(clean(r.historic).toUpperCase()==='H')g.historic=true;for(const qt of r.quarters)g.quarters.add(qt);if(isNSAID(r.drug))g.nsaid=true;else{if(r.drug)g.antibiotics.add(r.drug);g.maxDoses=Math.max(g.maxDoses,r.duration||0)}}
  ...
  const unmatchedCases=cases.filter(c=>!herdById.has(c.id));
  const historicUnmatchedCases=unmatchedCases.filter(c=>c.historic);
  const otherUnmatchedCases=unmatchedCases.filter(c=>!c.historic);
  const historicUnmatchedCowIds=new Set(historicUnmatchedCases.map(c=>c.id));
  const otherUnmatchedCowIds=new Set(otherUnmatchedCases.map(c=>c.id));
  const unmatchedCowCases=unmatchedCases.length;
  return{mastitisRows,antibioticRows,nsaidRows,cases,uniqueCows:byCow.size,repeatCows:repeatCowIds.length,repeatCowIds,repeatQuarterCows:repeatQuarterCowIds.length,repeatQuarterCowIds,combo,extended,txChanges,productCounts,quarters,timing,monthly,unmatchedCowCases,historicUnmatchedCases:historicUnmatchedCases.length,historicUnmatchedCows:historicUnmatchedCowIds.size,otherUnmatchedCases:otherUnmatchedCases.length,otherUnmatchedCows:otherUnmatchedCowIds.size};
}
...
function renderResult(r){
  ...
  els.caseSummary.innerHTML=metricsHtml([['Cows with mastitis case events',fmt(r.caseAnalysis.uniqueCows)],['Repeat cows',fmt(r.caseAnalysis.repeatCows),`${pct(r.caseAnalysis.repeatCows,r.peak)} of peak cows`],['Repeat cow + same quarter',fmt(r.caseAnalysis.repeatQuarterCows)],['Ab + NSAID combination cases',fmt(r.caseAnalysis.combo),`${pct(r.caseAnalysis.combo,r.caseAnalysis.cases.length)} of clinical case events`],['Extended antibiotic cases (>3 doses)',fmt(r.caseAnalysis.extended)],['Treatment change within same case',fmt(r.caseAnalysis.txChanges)],['Historic-animal mastitis cases (H)',fmt(r.caseAnalysis.historicUnmatchedCases),`${fmt(r.caseAnalysis.historicUnmatchedCows)} cow${r.caseAnalysis.historicUnmatchedCows===1?'':'s'} · included in season totals`],['Other unmatched mastitis cases',fmt(r.caseAnalysis.otherUnmatchedCases),r.caseAnalysis.otherUnmatchedCases?`${fmt(r.caseAnalysis.otherUnmatchedCows)} cow${r.caseAnalysis.otherUnmatchedCows===1?'':'s'} · check cow IDs / herd-file coverage`:'No unexplained unmatched cases']]);
